...
globalThis.HL_editmode = () => {
  globalThis.structure.mode = (
    document.getElementById("edit_mode") as HTMLInputElement
  ).value;
...
globalThis.HL_changeparent = (my_id: number) => {
  ...
  let str_newparentid = (
    document.getElementById("id_parent_change_" + my_id) as HTMLInputElement
  ).value;
  ...
  if (int_newparentid != 0) {
    ...
    if (parentOrdinal == null) error = 1;
    else if (
      !globalThis.structure.active[parentOrdinal] ||
      int_newparentid == my_id
    )
      error = 1;
    else {
  ...
  switch (error) {
    ...
    case 2:
      alert(
        "Circulaire referentie. Nieuwe moeder kan geen afstammeling van het te verplaatsen element zijn."
      );
      break;
    default:
...
globalThis.HL_cancelFilename = () => {
  ...
  settings.innerHTML =
    ...
    '</code><br><button style="font-size:14px" onclick="exportjson()">Opslaan</button>&nbsp;<button style="font-size:14px" onclick="HL_enterSettings()">Naam wijzigen</button>';
};
...
globalThis.HL_changeFilename = () => {
  ...
  if (regex.test(filename)) {
    globalThis.structure.properties.setFilename(
      (document.getElementById("filename") as HTMLInputElement).value
    );
    settings.innerHTML =
      "<code>" +
      globalThis.structure.properties.filename +
      '</code><br><button style="font-size:14px" onclick="HL_enterSettings()">Wijzigen</button>&nbsp;<button style="font-size:14px" onclick="exportjson()">Opslaan</button>';
  } else {
    globalThis.structure.properties.setFilename(
      (document.getElementById("filename") as HTMLInputElement).value + ".eds"
    );
    settings.innerHTML =
      "<code>" +
      globalThis.structure.properties.filename +
...
globalThis.HL_enterSettings = () => {
  ...
  if (settings == null) return;
  settings.innerHTML =
    '<input type="text" id="filename" onchange="HL_changeFilename()" value="' +
    globalThis.structure.properties.filename +
    '" pattern="^.*\\.eds$"><br><i>Gebruik enkel alphanumerieke karakters a-z A-Z 0-9, streepjes en spaties. <b>Eindig met ".eds"</b>. Druk daarna op enter.</i><br><button onclick="HL_cancelFilename()">Annuleer</button>&nbsp;<button onclick="HL_changeFilename()">Toepassen</button>';
};
...
globalThis.HLRedrawTreeHTML = () => {
  ...
  var output: string =
    globalThis.structure.toHTML(0) + "<br>" + renderAddressStacked();
...
function buildNewStructure(structure: Hierarchical_List) {
  ...
  structure.data[0].props.type_kabel_na_teller =
    CONF_aantal_fazen_droog + "x16";
  structure.data[0].props.differentieel_delta_amperage =
    CONF_differentieel_droog;
  itemCounter++;
...
globalThis.changeAddressParams = () => {
  globalThis.structure.properties.owner = (
    document.getElementById("conf_owner") as HTMLElement
  ).innerHTML;
  globalThis.structure.properties.installer = (
    document.getElementById("conf_installer") as HTMLElement
  ).innerHTML;
  globalThis.structure.properties.control = (
    document.getElementById("conf_control") as HTMLElement
  ).innerHTML;
  globalThis.structure.properties.info = (
    document.getElementById("conf_info") as HTMLElement
  ).innerHTML;
};
...
globalThis.toggleAppView = (type: "2col" | "3col" | "config" | "draw") => {
  ...
  if (
    ["2col", "draw"].includes(type) &&
    ["2col", "draw"].includes(lastview) &&
    type !== lastview
  )
    globalThis.undostruct.store();
};
